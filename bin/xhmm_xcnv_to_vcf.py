            ...
            for sample_id in sample_order:
                ...
                for mutation in mutations:
                    try:
                        ...
                        if float(q_some) >= 60 and float(q_exact) >= 60 and float(q_non_diploid) >= 60:
                            filter_status = 'PASS'
                        else:
                        ...
                        vcf_file.write(f"{chr_num}\t{start}\t.\t{ref}\t{alt}\t.\t{filter_status}\tEND={end};SVLEN={svlen};SVTYPE={cnv};Q_START={q_start};Q_STOP={q_stop};SVMETHOD=XHMM;STRANDS={strand}\tGT:Q_EXACT:Q_SOME:Q_NON_DIPLOID")
                            ...
                            if sample_id in mutations_by_sample:
                                ...
                                for sample_mutation in sample_mutations:
                                    ...
                                    if sample_mutation == mutation:
                                        vcf_file.write(f"\t{gt}:{float(q_some) if q_some else '.'}:{float(q_exact) if q_exact else '.'}:{float(q_non_diploid) if q_non_diploid else '.'}")
                                        mutation_found = True
                                        break
