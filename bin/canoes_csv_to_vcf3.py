...
def convert_canoes_csv_to_vcf(input_file, output_dir, combined_vcf_name, log_file):
    ...
    try:
        ...
        # Write all mutations to a single unified VCF file if any mutations are found
            ...
            for sample_id in sample_order:
                ...
                for mutation in mutations:
                    try:
                        ...
                        # Split the interval into start and end positions
                        start, end = map(int, interval.split(':')[1].split('-'))
                        svlen = end - start  # Calculate the length of the structural variant
                        ci_pos = (0, 0)  # Confidence interval around the position
                        ci_end = (0, 0)  # Confidence interval around the end
                        strands = '.'  # Direction of the reads (not provided, hence '.')

                        # Determine filter status based on length and quality score
                        ...
                        # Set genotype format values based on copy number (cn)
                        if cn == '3' or cn == '1':
                            format_values = '0/1'
                        elif cn == '2':
                            format_values = '0/0'
...
def write_vcf_file_per_sample(output_dir, mutations_by_sample, sample_order):
    for sample_id in sample_order:
        try:
            # Determine the filename for the current sample's VCF file
            sample_vcf_file = os.path.join(output_dir, f"{sample_id}.vcf")
            # Write the current sample's mutations to the VCF file
...
# Main function that defines command-line arguments and calls the conversion function
def main():
    parser = argparse.ArgumentParser(description="Convert CANOES CSV file to VCF format.")
    parser.add_argument("-i", "--input_file", required=True, help="Path to the CANOES CSV file")
    parser.add_argument("-d", "--output_dir", required=False, help="Directory to save the VCF files")
    parser.add_argument("-o", "--combined_vcf_name", required=True, help="Name for the unified VCF file")
    parser.add_argument("-l", "--log_file", required=False, help="Path to the log file for error reporting")
