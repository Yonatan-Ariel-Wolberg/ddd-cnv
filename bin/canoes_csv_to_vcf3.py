...
def convert_canoes_csv_to_vcf(input_file, output_file):
    ...
    with open(input_file, 'r') as csvfile:
        ...
        # Dictionary to store mutations by sample
        mutations_by_sample = {}
    ...
    # Write VCF files for each sample
    for sample_id, mutations in mutations_by_sample.items():
        vcf_filename = f"{os.path.splitext(output_file)[0]}_{sample_id}.vcf"
        write_vcf_file(vcf_filename, sample_id, mutations)

def write_vcf_file(output_file, sample_id, mutations):
    ...
    with open(output_file, 'w') as vcf_file:
        ...
        for mutation in mutations:
            try:
                ...
                vcf_file.write(f"chr{chr_num}\t{start}\t.\tN\t{cnv}\t.\t{filter_status}\tEND={end};SVLEN={svlen};SVMETHOD=CANOES;SVTYPE=CNV;CIPOS={ci_pos};CIEND={ci_end};STRANDS={strands}\tGT:Q_SOME\t{format_values}:{q_some if q_some else 0}\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Convert CANOES CSV file to VCF file for each sample.")
    ...
    parser.add_argument("output_file", help="Output VCF file prefix")
